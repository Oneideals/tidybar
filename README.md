...
**M0 技术验证完成**（4/4 通过），分层与逻辑已落地并有 143 条回归用例。枚举与 ⌘ 拖拽两个高危环节都已在真机打通；但 100 次全绿是在**自造 fixture 图标**上取得的，所以产品侧接管闸门（`isConfirmedSupportedOS`）仍未开放——现在它读得到你的图标，但不会移动任何一个。
...
```bash
swift build                       # 编译全部目标
swift run tidybar-checks          # 跑 143 条回归用例
...
| 冷启动到接管 | 2 s | 探针 3 次 1.69 / 1.78 / 1.81 s；**真 app 冒烟测得 2.80 s，已击穿预算**，列为进 M1 前必修 |
| 安装包 | 10 MB | **932 KB** |
...
① 冷启动接管压到 1s 内——真 app 已测得 2.80s 击穿 2s 预算，几乎全部成本在"遍历 90 个进程建 AX 连接"，
方向是只扫 accessory 进程 + 并发化；② 在用户真实图标上复跑 `--via-engine` 100 次闸门（破坏性实验，
需要你先松手别动鼠标并明确同意）；③ 进入 M1 功能面（三分区 + 收纳面板 + 搜索 + 首启向导）。
