...
**M0 技术验证完成**（4/4 通过），分层与逻辑已落地并有 144 条回归用例。枚举与 ⌘ 拖拽两个高危环节都已在真机打通；但 100 次全绿是在**自造 fixture 图标**上取得的，所以产品侧接管闸门（`isConfirmedSupportedOS`）仍未开放——现在它读得到你的图标，但不会移动任何一个。
...
| ⌘ 拖拽移动图标（合成事件） | **已真机验证**：fixture 图标 100/100、零卡键；真实菜单栏（35 图标）48 轮 / 96 次往返全绿。用户自己 App 的图标被拖走这条字面判据还没跑，暂不开放接管 |
...
```bash
swift build                       # 编译全部目标
swift run tidybar-checks          # 跑 144 条回归用例
